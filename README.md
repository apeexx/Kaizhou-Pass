...
- `takeout-merchant/` 和 `takeout-rider/` 当前已按项目统一主题重整为轻电商 + 本地生活特卖风格，使用既有本地真实图片资产展示门店、厨房、餐品、外卖打包和骑手配送场景。
...
- `messages.html`：消息，合并展示系统、预约、审核、收益等不需要回复的通知；用户私信单独展示为客户列表，点击客户可进入详细私信并支持输入追加消息、快捷回复和复制手机号提示。
